...
class ImportCSV(object):
    ...
    def recuperation_des_donnes(self):
        ...
        self._donnees_importees = pd.read_csv(
            self._fichier_CSV_a_importer,
            sep=self._separator,
            skiprows=self._nombre_de_lignes_a_ignorer,
        ...
        # Les deux lignes suivantes seront sans doute à supprimer une fois que la lecture du CSV en utilisant
        # l'option decimal = "." sera active
        ...
        self._donnees_importees["Montant(EUROS)"] = self._donnees_importees["Montant(EUROS)"].astype(float)
